...
pp_doublesaddle_BN = function(u,x,muhat){
  x = as.matrix(x)
  nx = dim(x)[2]
  x2 = x[,2:nx]
  
  
  that = optim(rep(0,nx),function(t)k(t,x,muhat)-u*t[1])$par
  
  what = sign(that[1])*sqrt(2*(-k(that,x,muhat)+that[1]*u)) 
  
  vhat = that[1]*sqrt(det(D2k(that,x,muhat))/det(D2k(rep(0,nx-1),x2,muhat)))
